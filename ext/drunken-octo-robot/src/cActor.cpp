...
CActor::CActor() : NodePath() { }

CActor::~CActor() { ; }

void CActor::load_actor(
...
{
  // preconditions
  assert(windowFrameworkPtr != NULL);
  //assert(!is_empty());
  ...
  for (AnimMap::const_iterator animMapItr = animMapPtr->begin();
       animMapItr != animMapPtr->end();
       ++animMapItr)
  {
  ...
  for (NodePathVec::iterator npItr = animNpVec.begin(); npItr < animNpVec.end();
       ++npItr)
  {
...
void CActor::load_anims(
    const AnimMap* animMapPtr, const string& filename, int hierarchyMatchFlags
)
{
  // precondition
  if (animMapPtr == NULL)
  ...
  if (animMapItr != animMapPtr->end())
  {
    ...
    for (NameVec::const_iterator nameItr = (*animMapItr).second.begin();
         nameItr != (*animMapItr).second.end();
         ++nameItr)
    {
      ...
      if (find_anim(*nameItr) == NULL)
      {
        ...
        PT(AnimControl)
        animPtr = tempCollection.find_anim(*nameItr);
        if (animPtr != NULL)
        {
          store_anim(animPtr, *nameItr);
          tempCollection.unbind_anim(*nameItr);
          nout << "Stored animation `" << *nameItr << "' from file `"
               << (*animMapItr).first << "'" << endl;
        }
      }
    }
         ...
         ++nameItr)
    {
      ...
      if (find_anim(*nameItr) == NULL)
      {
        ...
        PT(AnimControl)
        animPtr = tempCollection.get_anim(animIdx);
        if (animPtr != NULL)
        {
          store_anim(animPtr, *nameItr);
          ++animIdx;
...
// This function gives access to a joint that can be controlled via its NodePath
// handle. Think of it as a write to that joint interface.
//
// Return value:
...
NodePath CActor::control_joint(const string& jointName)
{
  ...
  PT(Character)
  characterPtr = DCAST(Character, characterNP.node());
  if (characterPtr != NULL)
  {
    ...
    if (characterJointPtr != NULL)
    {
      ...
      {
        if (characterPtr->get_bundle(i)
                ->control_joint(jointName, jointNp.node()))
        {
          foundJoint = true;
