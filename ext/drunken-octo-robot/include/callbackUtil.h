...
inline void define_key_t(WindowFramework* wf,
   ...
   if(wf == NULL)
      {
      nout << "ERROR: parameter wf cannot be null." << std::endl;
      return;
      }
...
//
template<class T, AsyncTask::DoneStatus (T::*func)(GenericAsyncTask *task)>
inline GenericAsyncTask* add_task_t(const std::string &name, T* obj)
   {
...
//
template <class T, void (T::*func)(const Event* event)>
inline bool add_hook_t(const std::string& event_name, T* obj)
   {
...
void EventCallbackStub(const Event* event, void* obj)
   ...
   if(obj == NULL)
      {
      nout << "ERROR: parameter obj cannot be null." << std::endl;
      return;
      }
