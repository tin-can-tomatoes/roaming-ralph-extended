...
void CBufferViewer::set_include(const pvector<Texture*>& x)
   {
   m_includeAll = false;
   m_include = x;
   m_dirty = true;
...
void CBufferViewer::set_include(const pvector<GraphicsOutput*>& x)
   {
   m_includeAll = false;
   m_include.clear();
   for(pvector<GraphicsOutput*>::const_iterator i = x.begin(); i != x.end(); ++i)
      {
...
void CBufferViewer::set_exclude(const pvector<Texture*>& x)
   {
   m_exclude = x;
   m_dirty = true;
   }

void CBufferViewer::set_exclude(const pvector<GraphicsOutput*>& x)
   {
   m_exclude.clear();
   for(pvector<GraphicsOutput*>::const_iterator i = x.begin(); i != x.end(); ++i)
      {
...
NodePath CBufferViewer::make_frame(int sizex, int sizey)
   ...
   std::unique_ptr<GeomVertexWriter> vwriter(new GeomVertexWriter(vdata, "vertex"));
   std::unique_ptr<GeomVertexWriter> cwriter(new GeomVertexWriter(vdata, "color" ));

   pvector<int> ringoffset;
...
AsyncTask::DoneStatus CBufferViewer::maintain_readout(GenericAsyncTask* taskPtr)
   ...
   for(pvector<NodePath>::iterator card = m_cards.begin(); card != m_cards.end(); ++card)
      {
      card->remove_node();
      }
   m_cards.clear();
   ...
   for(int iwin = 0; iwin < graphicsEnginePtr->get_num_windows(); ++iwin)
      ...
      for(int itex = 0; itex < win->count_textures(); ++itex)
         ...
         for(pvector<Texture*>::iterator i = m_include.begin(); i != m_include.end() && !isIncluded; ++i)
            {
         ...
         for(pvector<Texture*>::iterator i = m_exclude.begin(); i != m_exclude.end() && !isExcluded; ++i)
            {
   ...
   for(pvector<GraphicsOutput*>::const_iterator win = wins.begin(); win != wins.end(); ++win)
      {
