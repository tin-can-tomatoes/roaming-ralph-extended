...
{
   TextNodeProxy(const std::string& name, COnscreenText::TextStyle style);
...
COnscreenText::TextNodeProxy::TextNodeProxy(const std::string& name,
...
//        many parameters.
COnscreenText::COnscreenText(const std::string& name,
...
void COnscreenText::set_text(const std::string& text)
{
   m_textNode->set_text(text);
}

// Reimplementation of TextNode::append_text.
void COnscreenText::append_text(const std::string& text)
{
   m_textNode->append_text(text);
}

// Reimplementation of TextNode::get_text.
std::string COnscreenText::get_text() const
{
...
void COnscreenText::set_fg(const LColorf& fg)
{
...
void COnscreenText::set_bg(const LColorf& bg)
{
   if(bg[3] != 0)
   {
      // If we have a background color, create a card.
...
void COnscreenText::set_shadow(const LColorf& shadow)
{
   if(shadow[3] != 0)
   {
      // If we have a shadow color, create a shadow.
...
void COnscreenText::set_frame(const LColorf& frame)
{
   if(frame[3] != 0)
   {
      // If we have a frame color, create a frame.
